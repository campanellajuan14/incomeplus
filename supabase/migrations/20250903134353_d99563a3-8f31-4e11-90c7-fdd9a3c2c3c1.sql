...
-- 2. Users can update their own profile (except admin fields)
...
-- Only expose non-sensitive fields: username, user_type, account_status, created_at
-- Hide sensitive data: phone, company, bio, admin_notes, email preferences
CREATE POLICY "Public can view limited profile data for platform features" 
ON public.user_profiles 
FOR SELECT 
USING (
  ...
  -- Note: Actual field filtering is handled by security definer functions
  -- like get_safe_public_profile() to avoid exposing sensitive fields
);

-- Create additional security function to get safe public profile data
...
COMMENT ON TABLE public.user_profiles IS 'User profile data with RLS policies: Users can manage own profiles, admins can manage all, limited public read for platform features only';

-- Add helpful function for users to check their own profile access
...
AS $$
  SELECT (
    ...
    OR 
    -- Limited public access for active profiles (handled by other functions)
